...
#m1 = Image.open(r"C:\Users\Live wire\Desktop\bg.jpg")
#mr1 = ImageTk.PhotoImage(m1)
#photo =Label(a, image=mr1)
#photo.place(x=0,y=0)

a.config(bg="MistyRose")

title = Label(a,text="Welcome to login",width=20,fg="red",bg="MistyRose",font=("Palatino Linotype", 40))
title.pack()

name = Label(a,text="Username",width=20,fg="red",bg="MistyRose",font=("Palatino Linotype", 20))
name.place(x=450,y=200)

namedata = Entry(a,width=20,fg="red",bg="white",font=("Palatino Linotype", 20))
namedata.place(x=550,y=250)

password = Label(a,text="Password",width=20,fg="red",bg="MistyRose",font=("Palatino Linotype", 20))
password.place(x=450,y=300)

passworddata = Entry(a,width=20,fg="red",bg="white",font=("Palatino Linotype", 20),show="*")
passworddata.place(x=550,y=350)

def submit():
    name = namedata.get()
    pwd = int(passworddata.get())
    if name=="" or pwd=="":
        ...
        database="Mounika")
        cursor = db.cursor()
        q = f"insert into login values('%s','%d')"%(name,pwd)
        try:
            cursor.execute(q)
            db.commit()
            messagebox.showinfo("LOGIN", "Data inserted")
        except Exception as e:
            db.rollback()
            messagebox.showerror("LOGIN", "Data not inserted")
            print(e)
        a.destroy()
        import mounikaproject
            


submit = Button(a,text="Submit",width=19,fg="red",bg="white",font=("Palatino Linotype", 18),command=submit)
...
screenEntry["values"]=("select screen","skyline screen","ripple screen","space screen",
                       "orient screen","pixel screen")
